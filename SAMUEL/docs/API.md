...
- `POST /routes/preview-customers` — `{ customerIds, employeeIds, roundtrip?, date?, recordTrip?, originMode? }` (`originMode`: `EMPLOYEE_LAST` padrão ou `COMPANY`; 1–25 clientes, 1–8 funcionários com login; sem persistir; inclui `dayLoad` + `startOrigin`); ordem das paradas = duração OSRM (`/table`); se a tabela falhar, **mais perto → mais longe**; traçado OSRM `/route` e, só se falhar, trilha ACTIVE ou reta; `recordTrip: true` grava trilha em **todas** as rotas do lote (1+ clientes)
...
- `POST /routes/:id/start` — EMPLOYEE atribuído → `IN_PROGRESS`; **multipart** `file` (foto do odômetro, obrigatória) + `vehicleId`, `startOdometerKm`, `startFuelLevel`, `latitude`, `longitude`, `startNotes?`; trava o veículo (`IN_USE`; 409 `VEHICLE_IN_USE` se outra rota `IN_PROGRESS` no mesmo carro); reordena paradas pela duração OSRM (fallback: mais perto → mais longe); erro `ROUTE_ALREADY_ACTIVE` se já houver outra rota do funcionário
- `POST /routes/:id/reroute` — EMPLOYEE atribuído + rota `IN_PROGRESS`; body `{ latitude, longitude, reorderRemaining }`; recalcula geometry/steps a partir do GPS (OSRM até o pin; trilha ACTIVE só se o OSRM falhar); `reorderRemaining: true` = pendentes pela duração OSRM (fallback mais perto→mais longe); rate limit Redis 30/min (mesmo preview)
