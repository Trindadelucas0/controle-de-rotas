...
| Item | Valor |
|------|--------|
| Versão do sistema | 0.21.3 — Rua do mapa antes da trilha gravada |
| Última atualização | 25/09/2026 — ordem pela duração OSRM; trilha só se o mapa falhar |
...
Rotas
 ...
 ├── RECURSOS — /customers (não EMPLOYEE), /employees, /vehicles, /fuel, /costs (não EMPLOYEE)
...
| `/fuel`, `/fuel/new`, `/fuel/[id]` | `components/fuel/FuelPages.tsx` | `docs/screens/fuel.md` |
| `/costs` | `components/costs/CostsDashboardPage.tsx` | `docs/screens/costs.md` |
| `/field/*` | `components/field/*` | `field-*.md` (incl. `field-visit.md`, `field-fuel.md`) |

API: `apps/api/src/modules/*` (incl. `ops`, `fuel`, `costs`) — contratos em `docs/API.md`.
...
Consolidadas no `PRD.md`. Ops: não inventar métrica; Presence (Redis) ≠ Operational derivado ≠ RH; “em atendimento” = visita `IN_PROGRESS` após check-in; após check-out a visita vai para `COMPLETED`/`FAILED` e a parada da rota atualiza. **Custos:** gasto REAL = soma de abastecimentos ACTIVE; ESTIMADO usa consumo cadastrado × preço de referência da empresa; sem dado = NÃO CALCULÁVEL (`value` null). Total do cupom só no servidor. Odômetro do veículo não apaga o histórico (`odometer_readings`). EMPLOYEE não vê `/costs`.
...
2c. **Custos da frota (gestor):** Recursos → **Abastecimentos** registra litros, km, R$/L (o total não se digita) e foto do cupom. Recursos → **Custos** mostra o mês com selos REAL / ESTIMADO / NÃO CALCULÁVEL. Na **Empresa**, ADMIN informa preço de referência (só para estimativa) e se o comprovante é obrigatório. No detalhe do veículo aparecem consumo REAL e leituras de km; no painel Ver/Gerir da rota, o card de custo. Campo: em **Minha rota**, **Abastecer** abre o formulário curto (sem dashboard no celular).
3. **Campo:** no celular, a **página não dá zoom** (pinch, duplo toque ou teclado); só o **mapa** (Navegar, Mapa operacional, planejador, pin do cliente) amplia com pinça. O mini-mapa de Minha rota não dá zoom. O ideal é HTTPS + app na tela inicial e **Permitir** no aviso de localização. Em `http://IP` a tela abre com **NÃO ESTÁ EM HTTPS**; o GPS do navegador continua bloqueado, mas dá para **Iniciar rota** (ordem planejada, origem = 1ª parada). Login EMPLOYEE → capa com **Minha rota** e **Agenda** (sem Clientes) → Minha rota (card com **mini-mapa** da linha planejada; nome do cliente na parada **sem** link ao prontuário) → Iniciar → **Permitir localização** (GPS rápido; se falhar, **Continuar com origem planejada**) → Resumo → veículo (último km/combustível; **não** dá para pegar um carro que já está em rota) → checklist (km, combustível, **foto do odômetro**: **Tirar foto** abre a câmera traseira; **Galeria** escolhe arquivo) → Play → Navegar → perto da parada o banner **Chegando** ganha **Cheguei** → `/field/visits/[id]` → **Cheguei — chegada verificada** (GPS) → preencher **resultado**, **observações**, **fotos** (**Tirar foto** / **Galeria**; JPEG comprimido; obrigatória se “Realizada”) → opcional **remarcar próxima** → **Finalizar visita** (GPS). Volta à navegação na próxima parada pendente. **Concluir rota:** arraste o controle em **Minha rota** ou na **navegação** (Encerrar no mapa **não** conclui). Se a visita estiver aberta (Cheguei sem finalizar), o arraste fica desabilitado e aparece o aviso com link para a visita. Informe **km final**, combustível e **outra foto do odômetro** (sem texto de “fraude” no celular). O sistema pergunta a condição: se todas as paradas foram feitas **ou** a distância restante planejada for ≤ **500 m**, confirma e grava `COMPLETED`; se restar mais de 500 m, confirma **incompleta** (`INCOMPLETE`, paradas pendentes `SKIPPED`) e o gestor vê no painel + trilha GPS no mapa. Visita aberta bloqueia a conclusão. Com GPS ativo o celular envia posição a cada **5 s** (mesmo parado); se a rota tiver **Gravar viagem**, a amostragem fica ~**2 s** e o badge mostra **Gravando · N pts** (âmbar se houver pontos na fila de rede). Os pontos ficam numa fila local se a internet falhar e vão no **Cheguei** (e de novo no **Finalizar** se a trilha ainda não salvou). **Só com Gravar viagem** a navegação **e** a tela da visita (depois do Cheguei) mostram botões **Porteira / Ponte / Bifurcação / Estrada ruim** (GPS atual; na visita, se o GPS do toque falhar, usa o ponto do Cheguei; se a rede falhar, o toque fica guardado e reenvia). Sem Gravar viagem a navegação e a visita seguem sem esses botões. Marcos já gravados no cliente ficam salvos e aparecem em **qualquer** rota/mapa futura desse cliente como **ícones** (não “Por”/“Bif”); **toque no ícone** mostra o nome (Porteira, Ponte…) e quem marcou. Perto de um marco de **qualquer parada da rota** (~120 m), se a rota tiver **Gravar viagem**, o banner pergunta **Ainda existe porteira?** (ou o tipo): **Sim, permanece** ou **Não, retirar** (some do cliente; se a rede falhar, a retirada fica na fila). Sem Gravar viagem o banner continua só com **OK**. Não pergunta de novo o mesmo marco por ~5 min (nem logo após marcar). Se o GPS fino demorar, o app usa primeiro a posição de rede/Wi‑Fi e segue refinando — o overlay “GPS demorou demais” só aparece se as duas tentativas falharem (não por timeout curto sozinho). Se aparecer “rota em andamento”, o card com o arrastar para concluir está no topo de Minha rota — inclusive se a rota for de outro dia. No PC, `http://localhost` libera GPS de verdade (o browser trata localhost como seguro): a navegação **sempre recalcula no 1º GPS** a partir da sua posição (1ª parada = a mais rápida pelas ruas; se o mapa falhar, a mais perto em linha reta) e, se você sair da rua planejada (~50 m por ~2–3 s), redesenha o traçado sem embaralhar as paradas. No mapa, a **linha menta nasce no carro e vai só até a próxima parada** (os pinos seguintes continuam visíveis, sem traçado depois do destino atual); Tempo / Restante / ETA no HUD seguem o GPS e a velocidade (não as horas da viagem gravada). O banner mostra a **próxima virada** (rua) e a **faixa** (ícones ou texto), não só “Saia em direção a…” do trecho atual. Sem GPS (HTTP no celular) o recálculo, check-in e finalizar **não** rodam. GPS com a tela bloqueada / app em segundo plano o navegador pode pausar — mantenha o app aberto na viagem.
...
5. Pin no mapa → Adicionar à rota → `/routes?customerId=` pré-seleciona o cliente. No planejador **Por clientes**, escolha **Origem do cálculo**: **Última localização** (padrão — GPS ao vivo ou último ponto gravado do funcionário) ou **Empresa** (pin E). Km, estimativa e ordem das paradas saem dessa origem; **Voltar para a empresa no fim** continua no E. Sem GPS, o cálculo usa o E e avisa. Para mudar o pin E, vá em Configurações → Empresa. Em **Clientes**, CEP é opcional (fazendas): use Lat/Lng ou o pin. No planejador, marque **Gravar viagem** para densificar o GPS e gravar a trilha real até **cada** cliente no Cheguei (vale para **todas** as rotas do lote); o finalizar tenta de novo se o Cheguei falhou. **Cada cliente com trilha gravada** (ACTIVE) só entra no traçado se o mapa (OSRM) não responder; quando o mapa responde, a linha vai pelas ruas até o pin e não repete o GPS da gravação. A ordem das paradas é a duração das ruas (mais rápida); se a tabela do mapa falhar, volta a mais perto → mais longe em linha reta. Na navegação Tempo/Restante/ETA acompanham a posição e a velocidade ao vivo. Aba **Gravar cliente**: gestor escolhe funcionário, data e veículo e **Publicar missão de gravar** (no celular o botão fica acima da barra inferior) — sem lista de clientes. Aba **Região**: clique no mapa ou **Buscar endereço** (Enter/clique na sugestão posiciona o centro e o círculo; pins da busca no mapa). Raio padrão **5 km** (500 m–50 km); o slider não reenquadra. Nome opcional, funcionário/veículo/data → **Publicar missão**. Após publicar, o painel limpa (mantém a data) para lançar outra região. Origem da rota = centro (não o pin E). No campo o card chama **Gravar região** e mostra “Você foi designado para {nome} · raio N km” + mini-mapa do círculo. Play/Adicionar ponto/Finalizar iguais a Gravar cliente. Na navegação o mapa usa máscara escura fora do raio, borda laranja tracejada e **Ver região** (enquadra o círculo; **Centralizar** volta ao GPS); pinça pausa o acompanhamento. GPS fora do raio: aviso com seta e km até a borda, **não** bloqueia. Clientes já no círculo no planejador são só informativos. Campo `employees.region` **não** é usado. SUPERVISOR vê a aba e não publica. No campo: **Adicionar ponto** (nome obrigatório no GPS atual; GPS não para) e **Finalizar por completo** quando quiser (0 ou N pontos; não pede nome). Cadastro só com nome fica **Em aberto** (lápis no `/map`); funcionário dono e gestor completam depois. Placeholder interno da sessão não aparece no mapa nem na lista de clientes.
